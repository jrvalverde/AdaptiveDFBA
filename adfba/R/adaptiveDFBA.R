...
if (TRUE) {
	...
	# before doing the calculation we will apply any needed modifications
	    ...
	    #
            ...
            #	No wonder, as there shouldn't be any "Time" reaction
	...
	# in LP problem
        ...
        if (verboseMode > 3) {
            lplow <- getColsLowBnds(problem(lpmod), which(c(rep(TRUE, length(react_id(model))))))
            lpupp <- getColsUppBnds(problem(lpmod), which(c(rep(TRUE, length(react_id(model))))))
...
if (FALSE) {			#section is deemed OK
	    ...
	    fba.sol <- optimizeProb(model, algorithm="fba", retOptSol=FALSE)
            if (verboseMode > 2) {
                if (sol$obj != fba.sol$obj) {
                    cat(err, "lpobj =", sol$obj, "fbaobj =", fba.sol$obj, '\n')
                }
            }
            if (verboseMode > 2) {
		...
		for (i in 1:length(sol$fluxes)) {
                ...
                if (n > 0)
                    cat(err, 'DIFF', n, 'differences found between LP and FBA fluxes\n')
            }
	    sol <- fba.sol
            if (verboseMode > 2) {
                cat('\n')
	...
	if (method == "FBA") {	# should be 'else if' once the above code is OK
	    ...
	    # here we are supposed to do an FBA internally
	    #mtf.sol <- optimizeProb(model, algorithm="mtf") # should give a message saying FBA is computed
