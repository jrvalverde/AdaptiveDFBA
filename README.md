...
Coupled with knowledge of the status of the system, we can couple these changes to
current conditions, like, e.g. detect whenever glucose or pH falls below a given level (which we
cannot predict in advance) as if we had a sensor in a production tank, and apply the 
changes desired to the medium (e.g. as having sensors in a production tank and adding
more nutrients or removing producs/toxins when a suitable concentration is reached).
...
DFBA knows this. With ADFBA we can detect when the time is reached and modify the limits
of specific reactions in the model to activate/inactivate specific routes. Or that you
are modeling response to fever: environmental temperature will change and with it,
metabolic rates should be increased or reduced corresponsingly.
...
when a given metabolite of flux has surpassed a threshold and modify flux limits as required.
...
add/remove reactions. We can even define this progressively by changing limits for
the new reactions slowly increasing them through sucessive steps to simulate the
spread of the vector in the population.

This has proven most useful (as shown in the paper) to simulate heterologous protein
expression: TAT proteins secretion is activated as a stress response, meaning that it
cannot remain active all the time, it has to be activated only after the oxidative stress
response is triggered. Similarly, SEC protein secretion is active during exponential
growth, but becomes minor during stress conditions / static growth phases. You cannot
activate SEC at the start of a simulation and leave it as such: it must be canceled 
at the end of exponential growth, when TAT (which was inactive) must in turn be
...
day/night timetables or fever cycles), and one in which we do not know in advance and
will need to act in response to specific environmental/model conditions whenever
they occur.
...
concentration of other metabolites: Streptomyces will prefer to use amino acids over
glucose if they are present in sufficient concentration.

For key fluxes (exchange or internal fluxes), we have collected their predicted value as
environmental metabolite concentrations changed, for all metabolites, then we have tried to
fit them statistically, eliminate the less significant variables and obtained a simplified
formula that matched flux evolution in response to (indirectly related) environmental 
...
and substitute the flux values calculated by FBA by the ones obtained with our curve fits
or AI/ML models to see how well did the simulation using our predictions would emulate
...
value should be and substitute the FBA/MTF computed one by our prediction.
...
For instance: in the above paper we found an individual non-instantaneously optimal solution
...
evidence, instead showed that cells behave "subotimally": they relase Alanine early in time
...
alanine flux on the environment and our model knew that on exponential-phase medium, a
...
resources that do not have any immediate benefit and may indeed never be needed), so
that when the times of need come, they have a reserve to feed upon and enlarge their 
survival expectation. This means they won't multiply so fast during abundance, exponential
...
occupy the echological niche, but, as we all know, times of hardship are not so uncommon.
